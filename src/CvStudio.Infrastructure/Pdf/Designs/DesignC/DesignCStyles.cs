namespace CvStudio.Infrastructure.Pdf.Designs.DesignC;

public static class DesignCStyles
{
    // Farben Sidebar
    public const string SidebarBg = "#FFFFFF";
    public const string SidebarText = "#1F2937";
    public const string SidebarMuted = "#6B7280";
    public const string Cyan = "#00BCD4";
    public const string CyanDark = "#0097A7";
    public const string BlobColor = "#DBEAFE";
    public const string BlobAccentDark = "#1D4ED8";

    // Farben Main
    public const string MainBg = "#FFFFFF";
    public const string MainText = "#0F1C2E";
    public const string MainMuted = "#6B7280";
    public const string SectionLine = "#E5E7EB";
    public const string BulletColor = "#00BCD4";

    // Schriftgroessen
    public const float NameSize = 26f;
    public const float HeadlineSize = 10f;
    public const float SectionTitle = 9f;
    public const float BodyText = 8.5f;
    public const float SmallText = 7.5f;
    public const float SidebarLabel = 7.5f;
    public const float SidebarBody = 8f;

    // Abstaende
    public const float SectionGap = 10f;
    public const float ItemGap = 7f;
    public const float BulletIndent = 8f;
    public const float SidebarWidth = 170f;
}
